...
if ($search !== '') {
    $search_sql = "WHERE activity_logs.username LIKE ? OR activity_logs.action LIKE ? OR activity_logs.details LIKE ? OR activity_logs.id LIKE ? OR activity_logs.report_id LIKE ?";
    $search_param = '%' . $search . '%';
    ...
    $query = "SELECT activity_logs.*, users.department FROM activity_logs LEFT JOIN users ON activity_logs.username = users.username ORDER BY activity_logs.timestamp DESC LIMIT $per_page OFFSET $offset";
