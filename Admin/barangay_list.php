...
// Handle adding a new barangay
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['barangay_name']) && !isset($_POST['barangay_id'])) {
    $barangay_name = $_POST['barangay_name'];

    // Insert the new barangay into the database
    $insertQuery = "INSERT INTO barangays (barangay_name) VALUES (?)";
    $stmt = $conn->prepare($insertQuery);
    $stmt->bind_param('s', $barangay_name);
    $barangay_name = htmlspecialchars(strip_tags($_POST['barangay_name']));


    if ($stmt->execute()) {
        $_SESSION['message'] = 'New barangay added!';
        $_SESSION['message_type'] = 'success';
    } else {
        $_SESSION['message'] = 'Error deleting barangay. Please try again.';
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['barangay_name']) && isset($_POST['barangay_id'])) {
    ...
    $updateQuery = "UPDATE barangays SET barangay_name = ? WHERE barangay_id = ?";
    $stmt = $conn->prepare($updateQuery);
    $stmt->bind_param('si', $barangay_name, $barangay_id);

    if ($stmt->execute()) {
        $_SESSION['message'] = 'Barangay updated!';
        $_SESSION['message_type'] = 'success';
    } else {
        $_SESSION['message'] = 'Error updating barangay. Please try again.';
        $_SESSION['message_type'] = 'error';
    }
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_barangay'])) {
    ...
    if ($stmt->execute()) {
        $_SESSION['message'] = 'Barangay deleted!';
        $_SESSION['message_type'] = 'success';
    } else {
        $_SESSION['message'] = 'Error deleting barangay. Please try again.';
        $_SESSION['message_type'] = 'error';
...
// Retrieve all barangays from the database
...
$per_page = 10; // Number of barangays per page
$page = isset($_GET['page']) ? max(1, intval($_GET['page'])) : 1;
$offset = ($page - 1) * $per_page;

// Get total barangays count
$count_query = "SELECT COUNT(*) as total FROM barangays";
$count_result = mysqli_query($conn, $count_query);
$total_reports = mysqli_fetch_assoc($count_result)['total'];
$total_pages = ceil($total_reports / $per_page);

// Retrieve barangays for current page
$query = "SELECT * FROM barangays ORDER BY barangay_name LIMIT $per_page OFFSET $offset";
$result = mysqli_query($conn, $query);
$barangays = mysqli_fetch_all($result, MYSQLI_ASSOC);
...
<head>
   ...
   <aside class="sidebar">
        <nav>
            <ul>
                <li class = "archive-text"><h4><?php echo htmlspecialchars($settings['system_name'] ?? 'BUREAU OF FIRE PROTECTION ARCHIVING SYSTEM'); ?></h4></li>
   ...
   <header class="header">
    ...
    <div class="search-input-container">
        <form method="GET" style="display:inline;" id="searchForm" onsubmit="return false;">
            <input type="search" name="search" class="search-input" placeholder="Search..." autocomplete="on">
            ...
            <tr>
                ...
                <td class = "action-button-container">
                    <form action="barangay_list.php" method="POST" style="display:flex;">
                        ...
                        <button type="button" onclick="openEditModal(<?php echo $barangay['barangay_id']; ?>, '<?php echo htmlspecialchars($barangay['barangay_name']); ?>')" class="edit-btn"><i class="fa-solid fa-pen-to-square"></i></button>
                    </form>
                </td>
            </tr>
        <?php endforeach; ?>
...
<div id="addBarangayModal" class="report-details-modal">
    <div class="modal-content">
        <span class="close-btn" onclick="closeModal('addBarangayModal')">&times;</span>
        <h3>Add New Barangay</h3>
        <form id="addBarangayForm" method="POST" action="barangay_list.php">
            ...
            <button type="submit" class="action-button">Add Barangay</button>
        </form>
    </div>
</div>

<div id="editBarangayModal" class="report-details-modal">
    <div class="modal-content">
        <span class="close-btn" onclick="closeModal('editBarangayModal')">&times;</span>
        <h3>Edit Barangay</h3>
        <form id="editBarangayForm" method="POST" action="barangay_list.php">
            <input type="hidden" id="edit_barangay_id" name="barangay_id">
            <label for="edit_barangay_name">Barangay Name:</label>
            <input type="text" id="edit_barangay_name" name="barangay_name" required>
...
<div id="confirmDeleteModal" class="confirm-delete-modal">
            <div class="modal-content">
                ...
                <hr>
                <p>Are you sure you want to delete this fire type?</p>
                <form action="barangay_list.php" method="POST" id="deleteForm">
                    <input type="hidden" name="barangay_id" id="deleteBarangayId">
                    <button type="submit" name="delete_barangay" class="confirm-btn">Confirm</button>
                    <button type="button" onclick="closeModal('confirmDeleteModal')" class="cancel-btn">Cancel</button>
                </form>
            </div>
        </div>
...
<hr>
    ...
    function openEditModal(barangayId, barangayName) {
    ...
    // Show the modal
    document.getElementById('editBarangayModal').style.display = 'flex';
}
...
function confirmDelete(barangay_id) {
                // Set the fire type ID in the hidden input field of the delete form
                document.getElementById('deleteBarangayId').value = barangay_id;
    ...
    if (searchInput && tableBody) {
        ...
        searchInput.addEventListener('input', function() {
            ...
            searchTimeout = setTimeout(function() {
                ...
                fetch('barangay_ajax.php?search=' + encodeURIComponent(query))
