...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['selected_ids'], $_POST['selected_types'])) {
    ...
    if ($type === 'Fire Incident Report') {
        ...
        $stmt_log = $conn->prepare("INSERT INTO activity_logs (username, action, report_id, details) VALUES (?, 'restore', ?, ?)");
        $stmt_log->bind_param("sis", $username, $id, $details);
        $stmt_log->execute();
        $stmt_log->close();
        ...
        $stmt_log = $conn->prepare("INSERT INTO activity_logs (username, action, id, details) VALUES (?, 'restore', ?, ?)");
        $stmt_log->bind_param("sis", $username, $id, $details);
        $stmt_log->execute();
        $stmt_log->close();
    }
