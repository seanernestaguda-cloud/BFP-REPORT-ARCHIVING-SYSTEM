...
.select-multi-btn .fa-check-square{ color: blue; }
.select-multi-btn .fa-trash{ color: red; }
.select-multi-btn .fa-download{ color: green; }
.select-multi-btn:hover{ background-color: #f0f0f0; }
.select-multi-btn.active, .select-multi-btn:active { background-color: #f0f0f0; }
.no-caret-select{ border: none; }
.fa-arrow-up-wide-short{ color: #ffa700; }
...
<div id="successModal" class="success-modal">
    ...
    <script src = "../js/permitscript.js"></script>
    <script>
...
function singleDeleteHandler() {
    ...
    .then(data => {
        if (data.success) {
            const row = document.getElementById('report-row' + selectedToDelete[0]);
            if (row) row.remove();
            openSuccessModal();
        } else {
