...
function toggleActionDropdown(event) {
    event.stopPropagation(); // Prevent click event from propagating
    const actionDropdown = event.target.closest('.action-dropdown').querySelector('.dropdown-content');

    // Close other dropdowns if open
    closeAllDropdownsExcept(actionDropdown);

    // Toggle the action dropdown visibility
    actionDropdown.style.display = (actionDropdown.style.display === 'block') ? 'none' : 'block';
}
...
window.onclick = function (event) {
    if (!event.target.matches('.user-icon') && !event.target.matches('.action-btn')) {
